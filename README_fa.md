...
LogGram از سه بخش اصلی تشکیل شده است:

1.  **Logger API** (`logger_api.py`) - یک REST API مبتنی بر FastAPI که لاگ‌ها را ذخیره و مدیریت می‌کند
2.  **Telegram Bot** (`LogGram.py`) - یک ربات تلگرام که لاگ‌ها را از پروژه‌های شما دریافت کرده و به چت‌های تعیین شده ارسال می‌کند
